<?php
/* ----------------------------------------------------------------------------
 * Programa: rel_chamadas.php - Relatorio de Chamadas
 * Baseado no software cdr - voipinfo.org  - Arezqui Belaid
...
function monta_relatorio($acao)  {
  ...
  try {
      ...
      $sql_ctds = "SELECT ".$SELECT." FROM cdr "  ;
      $sql_ctds .= " LEFT JOIN ccustos on cdr.accountcode = ccustos.codigo " ;
      $sql_ctds .=  $CONDICAO ;
  ...
  $sql_chamadas = "SELECT count(userfield) as qtdade,".$SELECT." FROM cdr "  ;
